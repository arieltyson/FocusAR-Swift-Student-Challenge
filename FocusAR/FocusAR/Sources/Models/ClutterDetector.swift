    ...
    init() {
        ...
        guard
            let modelURL = Bundle.main.url(
                forResource: "CustomClutter",
                withExtension: "mlmodelc"
            )
        else {
            print(
                "Could not find compiled ClutterClassifier.mlmodelc in resources. Falling back to saliency detection."
            )
            return
        }
        ...
        do {
            ...
            print(
                "Error loading compiled model: \(error). Falling back to saliency detection."
            )
        }
    }
        ...
        if let vnModel = vnModel {
            ...
            do {
                ...
                if let result = request.results?.first
                    ...
                    let maxConfidence = salientObjects.map({
                        Float($0.confidence)
                    }).max()
                {
                    DispatchQueue.main.async {
